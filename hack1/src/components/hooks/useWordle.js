...
const useWordle = (solution) => {
    ...
    const solution_letter_set = (solution);
    ...
    const handleEnter = () => {
        ...
        let guess = [];
        for (let i=0 ; i<5 ; ++i){
            if (curGuess[i] === solution_letter_set[i]) {
                guess.push({char : curGuess[i], color:'green'})}
            else if (curGuess[i] in solution_letter_set) {
                guess.push({char : curGuess[i], color:'yellow'})
                }
            else {
                guess.push({char : curGuess[i], color:'grey'})}
            }
        ...
        if (curGuess === solution_letter_set){
            setIsCorrect(true);
        }
        ...
        for (let i=0 ; i<5 ; ++i){
            if (solution_letter_set[i] in usedChars.key()){
                continue;
            }
            setCurGuess(usedChars => usedChars + {solution_letter_set,[i] : 'grey'});
        }
